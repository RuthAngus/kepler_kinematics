...
stellar positions and velocities in equatorial, or International Celestial
Reference System (ICRS) coordinates into positions and velocities in
Galactocentric coordinates.
In other words, it converts positions and proper motions in RA and dec,
parallaxes/distances, and RVs into \x, \y, \z, \vx, \vy, \vz.
For stars without RVs we {\it inferred} their velocities by marginalizing over
their RVs.
...
using {\tt numpy} and {\tt Theano} \citep{citations}.
...
At each set of model parameters the likelihood is calculated, using, a
Gaussian likelihood function and multiplied by a prior to produce the
posterior probability: the probability of those model parameters given the
data.
The prior is described below.
...
using the Kepler targets {\it with RV measurements}.
...
One of the main features of the RV selection functions is brightness: Gaia
DR2 RVs are only available for stars brighter than around 13th magnitude, and
LAMOST DR5 RVs for stars brighter than around 17th magnitude, and APOGEE DR16
RVs for stars brighter than XXXth magnitude.
...
13), one from the faintest half ($G$ $>$ 13), and one from all stars with RVs.
...
massive, younger, more evolved, or closer to the Sun on average than faint
stars.
...
non-Gaussian -- more so than the faint stars. \racomment{why?}.
...
in our sample.
As a result, inferred velocities that are strongly prior-dependent -- (\ie
especially those in the \y-direction) will have some slight inaccuracies
baked-in.
...
The \vy\ velocities are a slightly different story and should be used with
caution.
